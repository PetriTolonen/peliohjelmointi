...
GameRunningState3::GameRunningState3(GameApp* app) : GameState(app), amoutOfPegs(0), victoryState(false)
{
	...
	esLogMessage("Init... %d", cc++);
	m_map->loadMapFile("assets/level3.tmx", componentFactory2);
	...
	// Peg amount text
	GameObject* text1 = createTextGameObject("Pegs left to win in life: " + std::to_string(amoutOfPegs - 1));
...
GameRunningState3::~GameRunningState3()
{
	delete componentFactory2;
}

bool GameRunningState3::update(ESContext* ctx, float deltaTime)
{
	...
	m_map->findGameObjectByName("Text")->getComponent<TextComponent>()->getText()->setText("Pegs left to win in life: " + std::to_string(amoutOfPegs - 1));
...
void GameRunningState3::draw(ESContext* ctx)
{
